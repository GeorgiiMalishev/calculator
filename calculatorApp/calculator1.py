...
if __name__ == "__main__":
    ...
    input_data = {
        'Общее факт среднее кол-во файлов в месяц': 10790,
        ...
        'Общее кол-во новых пользователей (УЗ)': 100000,
    ...
    formulas = {
        ...
        'Факт максимальное кол-во файлов 180 часов': Formula(
            lambda x, y: x * y, ['Факт кол-во машин 180 часов', 'Максимальное кол-во файлов в месяц 180 часов']
        ),
        'Факт максимальное кол-во файлов 168 часов': Formula(
            lambda x, y: x * y, ['Факт кол-во машин 168 часов', 'Максимальное кол-во файлов в месяц 168 часов']
        ),
        'Факт максимальное кол-во файлов 79 часов': Formula(
            lambda x, y: x * y, ['Факт кол-во машин 79 часов', 'Максимальное кол-во файлов в месяц 79 часов']
        ),


        # Формула для общей суммы всех "Факт максимальное кол-во файлов"
        ...
        'День факт разница нагрузки': Formula(
            lambda total, day_avg: total - day_avg,
        ...
        'День/пр/вых факт разница нагрузки': Formula(
            lambda max_prazdn, day_night_avg: max_prazdn - day_night_avg,
            ['Факт максимальное кол-во файлов 180 часов праздники/вых',
             'День/пр/вых факт среднее кол-во файлов в месяц']
        ),

        'Ночь/пр/вых факт разница нагрузки': Formula(
            lambda max_night, night_avg: max_night - night_avg,
            ['Факт максимальное кол-во файлов 180 часов', 'Ночь/пр/вых факт среднее кол-во файлов в месяц']
        ),

        'День факт нагрузка в %': Formula(
            lambda day_avg, total: (day_avg / total) * 100 if total != 0 else 0,
            ['День факт среднее кол-во файлов в месяц', 'Общая сумма факт максимальное кол-во файлов']
        ),

        'День/пр/вых факт нагрузка в %': Formula(
            lambda day_max, day_avg: (day_avg / day_max) * 100 if day_avg != 0 else 0,
            ['Факт максимальное кол-во файлов 180 часов праздники/вых', 'День/пр/вых факт среднее кол-во файлов в месяц']
        ),
        ...
        'Факт нехватка машин 168 часов': Formula(
            ...
            ['День факт нагрузка в %', 'Общая сумма факт максимальное кол-во файлов', 'Максимальное кол-во файлов в месяц 168 часов']
        ),

        'Факт нехватка машин 79 часов': Formula(
            lambda G4, sum, L5, F5, F6: 0 if (G4 / (sum + L5 * F5)) < 86 else math.ceil(((G4 / (sum + L5 * F5)) - 86) * (sum + L5 * F5) / 86 / F6),
            ['День факт среднее кол-во файлов в месяц', 'Общая сумма факт максимальное кол-во файлов', 'Факт нехватка машин 168 часов', 'Максимальное кол-во файлов в месяц 168 часов', 'Максимальное кол-во файлов в месяц 79 часов']
        ),

        'Ср колво файлов новых УЗ в месяц день/пр/вых': Formula(
            lambda N4: (N4 * 1.3) * 0.15,
            ['Общее кол-во новых пользователей (УЗ)']
        ),

        'Ср колво файлов новых УЗ в месяц ночь/пр/вых': Formula(
            lambda N4: (N4 * 1.3) * 0.27,
            ['Общее кол-во новых пользователей (УЗ)']
        ),

        'Ср колво файлов новых УЗ в месяц день': Formula(
            lambda N4, O8, O7: N4 * 1.3 - O8 - O7,
            ['Общее кол-во новых пользователей (УЗ)', 'Ср колво файлов новых УЗ в месяц день/пр/вых', 'Ср колво файлов новых УЗ в месяц ночь/пр/вых']
        ),
        ...
        'План нагрузка в % ночь/пр/вых': Formula(
            lambda a, b: b / a * 100,
            ['Факт максимальное кол-во файлов 180 часов праздники/вых', 'Ср колво файлов с учетом новых УЗ в месяц ночь/пр/вых']
        ),

        'Планируемая нехватка машин пр/вых': Formula(
            lambda T8, R7, F7: 0 if T8 < 86 else math.ceil((R7 * (T8 - 86)) / 86 / F7),
            ['План нагрузка в % ночь/пр/вых', 'Факт максимальное кол-во файлов 180 часов праздники/вых', 'Максимальное кол-во файлов в месяц 180 часов пр/вых/ночь']
        ),
        ...
        '(план)Факт максимальное кол-во файлов 180 часов': Formula(
            lambda x, y: x * y, ['(план)Факт кол-во машин 180 часов', 'Максимальное кол-во файлов в месяц 180 часов']
        ),

        'сумм R4:R6': Formula(
            lambda a, b, c: a + b + c,
            ['(план)Факт максимальное кол-во файлов 180 часов', 'Факт максимальное кол-во файлов 168 часов', 'Факт максимальное кол-во файлов 79 часов']
        ),
        ...
        'Планируемая разница нагрузки день/пр/вых': Formula(
            lambda a, b: a - b,
            ['Факт максимальное кол-во файлов 180 часов праздники/вых', 'Ср колво файлов с учетом новых УЗ в месяц день/пр/вых']
        ),
        ...
        'Планируемая нагрузка в % день/пр/вых': Formula(
            lambda a, b: a / b * 100,
            ['Ср колво файлов с учетом новых УЗ в месяц день/пр/вых', 'Факт максимальное кол-во файлов 180 часов праздники/вых']
        ),

        'Планируемая нагрузка в % ночь/пр/вых': Formula(
            lambda a, b: a / b * 100,
            ['Ср колво файлов с учетом новых УЗ в месяц ночь/пр/вых', 'Факт максимальное кол-во файлов 180 часов праздники/вых']
        ),
        ...
        'Планируемая нехватка машин 168 часов': Formula(
            lambda T4, sum, F5: 0 if T4 <= 0.86 else round((sum * (T4 - 86)) / 86 / F5, 0),
        ...
        'Планируемая нехватка машин 79 часов': Formula(
            lambda O4, sum, U5, F5, F6: 0 if (O4 / (sum + U5 * F5)) <= 86 else
            math.ceil(((O4 / (sum + U5 * F5)) - 86) * (sum + U5 * F5) / 86 / F6),
            ['Ср колво файлов новых УЗ в месяц день', 'сумм R4:R6', 'Планируемая нехватка машин 168 часов', 'Максимальное кол-во файлов в месяц 168 часов', 'Максимальное кол-во файлов в месяц 79 часов']
        ),
